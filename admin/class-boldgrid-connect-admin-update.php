 ...
 */
class Boldgrid_Connect_Admin_Update {
	/**
	...
	public function __construct() {
		$this->configs = Boldgrid_Connect_Service::get( 'configs' );
	}
	...
	public function custom_plugins_transient_update( $transient ) {
		...
		$is_force_check = isset( $_GET['force-check'] ); // phpcs:ignore WordPress
		...
		if ( ! $version_data || ( $is_force_check && $is_data_old ) ) {
			...
			if ( ! empty( $version_data ) && 200 === $version_data->status &&
				! empty( $version_data->result->data ) ) {
					...
					set_site_transient(
						$this->configs['plugin_transient_name'], $version_data,
						8 * HOUR_IN_SECONDS
					);
			} else {
		...
		if ( 'plugin-install.php' === $pagenow && isset( $_GET['plugin'] ) && // phpcs:ignore WordPress.CSRF.NonceVerification.NoNonceVerification
			$this->configs['plugin_name'] === $_GET['plugin'] ) { // phpcs:ignore WordPress.CSRF.NonceVerification.NoNonceVerification
			...
			$transient->download_link = $this->configs['asset_server'] .
				...
				$version_data->result->data->asset_id .
			...
			if ( ! empty( $version_data->result->data->compatibility ) &&
				( $compatibility = json_decode( $version_data->result->data->compatibility, true ) ) ) { // phpcs:ignore
					$transient->compatibility = $version_data->result->data->compatibility; // phpcs:ignore
			}
			...
			if ( ! empty( $version_data->result->data->tags ) &&
				( $tags = json_decode( $version_data->result->data->tags, true ) ) ) { // phpcs:ignore
					$transient->tags = $version_data->result->data->tags; // phpcs:ignore
			}
	...
	public function site_transient_update_plugins( $value ) {
		...
		if ( 'update-core.php' !== $pagenow || ! isset( $_GET['force-check'] ) ) { // phpcs:ignore WordPress.CSRF.NonceVerification.NoNonceVerification
			return $value;
		}
